...
`BEAT-01` requires one pure function to return the existing active beat or deterministically choose the next beat from persisted state, meeting context, roster, and transcript facts. `BEAT-02` requires the chosen beat to be persisted before exposure and protected by a versioned compare-and-set write. `BEAT-03` requires one canonical stored effect per beat id, including quality-accepted generated and crisis entries, acknowledged room cues, user-share interpretation, and the complete close response. `BEAT-04` requires the client to render server beats without hardcoded round or speaker sequencing. `BEAT-05` requires refresh at every user gate, after every generated or cue entry, and after close to restore transcript and reflection without growth or loss. `BEAT-06` requires intake- and user-share-triggered crisis interruption and recovery to preserve a canonical source and truthful resume point. `BEAT-07` requires a fresh persisted empty-chair moment that follows all prompt and quality constraints. `BEAT-08` requires the room's topic choice and completion state to survive refresh.
...
- `BEAT-D` injects the server-supplied transition time and adds completion, deterministic optional branches, listening-only, both canonical crisis-trigger sources, the intake-trigger handled marker, resume behavior, and the direct close-to-finished path to the pure engine. It stops at core tests.
...
- `BEAT-F` adds the database contract and migration `20260912_000005_server_owned_meeting_beats.sql`, including canonical user-share analysis, durable room-cue fields, the generation-run ledger, the unique close-run ledger, and atomic close finalization. It updates validators but does not author fixtures, mock behavior, contract tests, or adapter queries.
- `BEAT-G` adds `probes/serverOwnedMeetingBeatsProbe.mjs`, runs it against the real local Supabase stack established by the privacy plan, and writes redacted version, beat, cue, generation-claim, analysis, close-claim, and atomic-finalization captures. It stops if a real probe cannot run and never hand-authors substitute fixtures.
- `BEAT-H` implements the fixture-backed mock's versioned phase, beat lookup, cue persistence, generation claims, unique-retry, first-analysis-wins, atomic topic, and atomic close-finalization behavior, then adds contract tests for schema conformance and mock fidelity.
- `BEAT-I` implements the captured behavior in the Supabase adapter with focused version-win, version-conflict, stored-cue, generation-claim, stored-beat, user-analysis race, close-claim winner, in-progress, atomic-finalization, and completed-result tests.
...
- `BEAT-N` migrates only `close/+server.ts` to the close-run claim, server-loaded transcript context, and atomic close finalization. It stops when concurrent, completed-retry, expired-lease recovery, and crash-boundary tests pass.
...
- `BEAT-R` adds a fixture-backed scripted scenario to the existing Grok mock, installs it through the preview-process composition, builds the refresh matrix in the route integration suite and `app/tests/e2e`, and removes the resolved replay item from `DEFERRED.md` only if every case passes.
...
Add `claimBeatGeneration`, `renewBeatGeneration`, `completeBeatGeneration`, and `failBeatGeneration`, or equivalent names with the migration's semantics. A character, crisis-support, or room-moment route checks for a stored share first, then acquires the claim before invoking the model. Only the current token can renew, complete, or fail. Completion stores the canonical share id; a completed retry loads it. Use a bounded lease comfortably longer than an ordinary provider call and renew it during generation and validation. No candidate content leaves the server before quality acceptance and persistence, and a competitor receives no generated chunks from a request that did not acquire the claim.
...
Expect the real local probe to capture one version winner, one cue/beat-row winner, one generation claimant with renewal, one canonical user analysis, one close claimant, meeting-scoped callback keys, token rejection, atomic-finalization rollback, the jointly stored finished phase and completed result, completed replay, and lease recovery before fixture, mock, contract, and adapter checks pass. If the real probe cannot run, stop the database-dependent milestones and record the blocker. Do not apply the migration to the unavailable production tenant as part of this local gate.
...
Expect the full browser flow and the dedicated refresh cases to pass against the server-process auth, database, and scripted Grok mocks with no provider calls. Check that the page no longer contains the retired sequence functions:
...
2026-09-13: Revised the plan after PR review to add durable generation and close claims, atomically finalize close phase and result, persist room cues, namespace callback effects by meeting, hydrate the stored summary, group legacy-route and renderer cutover, derive user-share and expansion decisions from persisted server state, represent intake and share crisis sources canonically, quality-gate crisis and empty-chair output, include close-summary prompt repair, install deterministic Grok fixtures in the preview composition, remove the uncompletable reflection gate, inject transition time from the clock seam, restore real-probe-before-fixture ordering, and point browser verification at Playwright's configured test directory.
